...
public class RecordProcessorService {
	...
	public Boolean processRecord(PayBatchRecord record, String glAccount) {
		...
		try {
			...
			// Step 2: Validate Customer (Read + Business Logic)
			log.info("│ → Step 2: Validating Customer for payment → customerId={}", customerId);

			boolean allowed = customerReadClient.readCustomer(customerId, record);
			...
			log.info("│ → Step 3: Writing Financial Allocation for recordId={} and customerId={}", record.getId(),
					customerId);
